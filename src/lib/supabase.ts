...
-- ==============================================================================
-- FUNÇÃO RPC ATÓMICA DE FINALIZAÇÃO DE VENDA & BAIXA DE STOCK (CONCORRÊNCIA E AUDITORIA)
-- ==============================================================================
...
BEGIN
    ...
    -- Garantia de idempotência contra re-tentativas de rede
    SELECT EXISTS (
    ...
    -- Decrementar stock com bloqueio FOR UPDATE
...
/**
 * Script SQL para criação da função RPC 'finalizar_venda_atomica'
 * com bloqueio pessimista (FOR UPDATE) no stock para prevenir concorrência e sobre-venda.
 */
export const SQL_RPC_ATOMIC_SALES = `-- ==============================================================================
-- FUNÇÃO RPC ATÓMICA DE FINALIZAÇÃO DE VENDA & BAIXA DE STOCK (POSTGRESQL / SUPABASE)
-- ==============================================================================
CREATE OR REPLACE FUNCTION public.finalizar_venda_atomica(
    p_venda JSONB,
    p_itens JSONB
...
BEGIN
    ...
    -- Inserir venda na tabela public.vendas
    INSERT INTO public.vendas (
        id, company_id, store_id, terminal_id, invoice_number, invoice_type,
        date, customer_id, customer_name, customer_tax_number,
        items, subtotal, discount_total, tax_total, total,
    ...
    -- Baixar stock com bloqueio pessimista (FOR UPDATE)
    FOR v_item IN SELECT * FROM jsonb_array_elements(COALESCE(p_itens, '[]'::jsonb))
    LOOP
        v_prod_id := v_item->>'productId';
        v_qty_to_deduct := COALESCE((v_item->>'quantity')::numeric, 1);
